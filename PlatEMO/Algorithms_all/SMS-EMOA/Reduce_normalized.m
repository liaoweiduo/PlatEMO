function [Population,FrontNo] = Reduce_normalized(Population,FrontNo,r)
% Delete one solution from the population
% normalized before operation
...
%--------------------------------------------------------------------------
    ...
    %% normalization
    MaxValues = max(Population.objs);
    MinValues = min(Population.objs);
    totalPopObj = (Population.objs - MinValues) ./ (MaxValues - MinValues);

    %% Identify the solutions in the last front
    FrontNo   = UpdateFront(totalPopObj,FrontNo);
    LastFront = find(FrontNo==max(FrontNo));
    PopObj    = totalPopObj(LastFront,:);
    [N,M]     = size(PopObj);
...
%     %% de-normalization
%     Population = PopObj .* (MaxValues - MinValues) + MinValues;
end