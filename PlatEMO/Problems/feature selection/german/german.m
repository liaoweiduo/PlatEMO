...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function obj = german()
            ...
            file = fullfile(fileparts(mfilename('fullpath')),'german.mat');
            if exist(file,'file') == 2
                load(file,'train','test','accuracy');
            else
                fid = fopen('german.data-numeric','rt');
                FormatString='';
                for i = 1:obj.Global.D
                    FormatString = [FormatString, '%4f '];
                end
                FormatString = [FormatString, '%4f'];
                n=1000;%读取数据的行数
