...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function obj = sonar()
            ...
            file = fullfile(fileparts(mfilename('fullpath')),'sonar.mat');
            if exist(file,'file') == 2
                load(file,'train','test','accuracy');
            else
                filename = 'sonar';
                fid = fopen([filename, '.all-data'],'rt');
                FormatString='';
                for i = 1:60
                ...
                class = data_all{end};
                n1 = find(strcmp(class, 'R'));
                n2 = find(strcmp(class, 'M'));
                class = ones(n,1);
                class(n1) = ones(size(n1,1),1) * 1;
                class(n2) = ones(size(n2,1),1) * 2;
                data = cell2mat({class,data_all{1:end-1}});
                fclose(fid);
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for DecIndex = 1:N
                if nsfs(DecIndex) ~= 0
                    ...
                    Mdl = fitcknn(obj.train(:,selectedFeatureList+1),obj.train(:,1),'NumNeighbors',5,'Standardize',1);
