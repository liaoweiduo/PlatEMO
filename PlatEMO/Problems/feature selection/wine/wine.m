...
% in each of the 3 types of wines.
...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function obj = wine()
            ...
            file = fullfile(fileparts(mfilename('fullpath')),'wine.mat');
            if exist(file,'file') == 2
                load(file,'train','test','accuracy');
            else
                fid = fopen('wine.data','rt');
                FormatString='%f %f %f %f %f %f %f %f %f %f %f %f %f %f';
                n=178;%读取数据的行数
                data=textscan(fid,FormatString,n,'delimiter',','); %以逗号为数据的分隔符
                data=cell2mat(data);%cell型数组转换为普通数组
                fclose(fid);
