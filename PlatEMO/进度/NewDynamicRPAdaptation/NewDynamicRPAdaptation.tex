...
a linearly decreasing mechanism and two static mechanisms. The results demonstrate the effectiveness of the proposed mechanism.
\end{abstract}

\begin{IEEEkeywords}
reference point; SMS-EMOA; hypervolume; evolutionary multi-objective optimization; 
...
one of the active research areas is the development of performance indicators for EMOAs. 
...
\end{table*}
\begin{table*}[!t]\footnotesize
    \caption{HV mean and standard deviation over 20 independent runs for minus-DTLZ, minus-WFG and MPDMP.} %加粗是好，灰色是坏
    \label{table_itri}
    \centering
    \begin{tabular}{ccccccc}
      \toprule
...
for that their values of $r$ are both 10 before 4,500 evaluations. % ($t_{Converged}$)
