...
The specification of reference point is one of the important parts in hypervolume-based algorithm.
In the source code of PlatEMO, the value of $r$ is chosen as follow: 
1.1(SMS-EMOA\cite{smsemoa}) and (HypE\cite{HypE}). % and 10(FV-EMOA\cite{FVEMOA}). 
...
\begin{figure}[!t]
  ...
  \caption{
    ...
    The first 6 dimensionalities are all 0 for the solutions(\ref{nud:b}). 
  }
  \label{nud}
\end{figure} 
...
\section{Existing Dynamic Reference Point Specification Mechanism}
...
the mechanisms are going to be tested with the algorithm SMS-EMOA\cite{smsemoa}.
...
\begin{table*}[!t]\footnotesize
    \caption{HV mean and standard deviation over 20 independent runs for corresponding minus-versions and MPMP.} %加粗是好，灰色是坏
    \label{table_itri}
    \centering
    \begin{tabular}{ccccccc}
      \toprule
...
SMS-EMOA-10(the SMS-EMOA\cite{smsemoa} algorithm with the reference point adaptation with $r=10$),
SMS-EMOA-Opt(the SMS-EMOA algorithm with the reference point adaptation with $r=1+1/H$),
...
We obtain the hypervolume results after $10000$ evaluations for each algorithm. 
The computational results of the hypervolume metric(reference point = $(1+1/H)*$PF) is shown in TABLE \ref{table_tri} and TABLE \ref{table_itri}. 
...
we can not tell the differences among the three algorithms in TABLE \ref{table_tri} except SMS-EMOA-Opt,
for that the Wilcoxon signed-rank sum tests show that almost all the results from SMS-EMOA-10 and SMS-EMOA-LD are not significantly different from SMS-EMOA-CD
(except 2 results from SMS-EMOA-LD are better 
and 1 result from SMS-EMOA-10 is worse than that from SMS-EMOA-CD). 
The results of SMS-EMOA-Opt clearly show the bad searching behavior when applying $r=1+1/H$ all the time, 
which is discussed in Section III(9 out of 13 results in SMS-EMOA-Opt are significantly worse than SMS-EMOA-CD). 

In the results of inverted-triangular pareto front problems (minus-DTLZ1-4, minus-WFG1-9 and MPDMP), 
SMS-EMOA-10 performs almost the worst in all experiments in average, 
...
The reason is that the $r$ is $10$ all the process of SMS-EMOA-10 algorithm. 

% Comparing with SMS-EMOA-CD, the final hypervolume values of SMS-EMOA-Opt and SMS-EMOA-LD are not significantly different. 
% And it is difficult to say SMS-EMOA-CD is better than SMS-EMOA-LD or vice versa,
