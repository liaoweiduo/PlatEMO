...
the hypervolume (HV) indicator is one very popular performance indicators. 
It is not only used for evaluating the performance of EMO algorithms (EMOAs) 
but also adopted in EMOAs for selection (e.g., SMS-EMOA). 
The reference point specification for HV calculation strongly affect the performance of SMS-EMOA. 
Thus, the reference point specification should be carefully treated in SMS-EMOA. 
In this paper, the importance of the dynamic specification of the reference point in SMS-EMOA is explained first. 
...
In Evolutionary Multi-objective Optimization (EMO) field, 
a popular research issue is the development of performance indicators. 
...
Among them, the HV indicator is treated as one very important performance indicator,
for that it is not only used for measuring the performance of EMO algorithms (EMOAs), 
...
For example, HypE estimates HV by using Monte Carlo simulation technique\cite{HypE}; 
...
FV-MOEA utilizes the fact that rather than determined by the whole solution set, 
the HV contribution of a solution is only determined by partial solutions\cite{FVEMOA}. 
Recently, an improved SMS-EMOA has been proposed and it reduces the number of HV calculations by the adaptive resource allocation \cite{ismsemoa}. 
...
Recently, it has been reported that the optimal distribution for HV maximization of a solution set 
is influenced by the reference point position in inverted-triangular Pareto fronts\cite{hisao:RPhowtoSpecify, hisao:RPspecify, hisao:RPexplanation}. 
...
In the early stage of the multi-objective evolution, 
we suggest to set the reference point far from the estimated nadir point to enhance the solution diversity. 
In the final stage of the multi-objective evolution, 
the reference point should be specified properly to obtain a uniformly distributed solution set over the entire Pareto front.
...
On some specific problems (e.g., Distance Minimization Problems\cite{dmp}), 
our weak convergence detection mechanism outperforms the linearly decreasing mechanism. 

The remainder of this paper is organized as follows. 
The explanations of the original reference point specification in SMS-EMOA and the one proposed in \cite{hisao:RPhowtoSpecify} are in Section II. 
% we briefly introduce the basic idea of reference point specification by a simple example in Section II. 
Then, we examine the specification of the reference point in two stages of multi-objective evolution by SMS-EMOA, 
...
Some examples of obtained solution sets by SMS-EMOA are shown in Fig. \ref{rpa2}. 
Using a faraway fixed reference point, 
nearly all solutions of the three-objective inverted DTLZ1 problem \cite{invertedDTLZ1} are on the boundaries of the Pareto front in Fig. \ref{rpa2} (a). 
In Fig. \ref{rpa2} (b), by appropriately specifying the reference point at each generation 
(e.g., by Eq. (\ref{frpa1}) shown later in this section), 
we obtain a solution set which is well-distributed. 
However, the specification of the reference point does not strongly affect 
the finally obtained solution set on triangular Pareto fornt problems as in Fig. \ref{rpa2} (c) and (d) for the three-objective DTLZ1 problem. 
The effect of specifying the location of the reference point on the distribution of solutions for HV maximization 
is further discussed in \cite{hisao:RPexplanation, hisao:RPspecify, hisao:dynamic}.
...
\end{equation}
where $\boldsymbol R \in \mathbb{R}^m$ is the reference point in each generation ($m$ is the number of objectives), 
and $\boldsymbol N \in \mathbb{R}^m$ is the estimated nadir point of the last front from the current population, 
after the non-dominated sorting. 
...
However, fixing the $r$ value to 1.1 is not recommended, especially for problems which have inverted-triangular Pareto fronts
...
This is due to the reference point location does not strongly influence the calculation of HV contribution 
when HV-based algorithms are applied to triangular Pareto fronts test problems. 
...
an appropriate value of $r$ was suggested from a point of view of fair HV-based performance comparison of EMO algorithms. 
The suggested $r$ is: 
\begin{equation}\label{eod}
  r=1+\frac{1}{H},
\end{equation}
where $H$ is used to generate uniformly distributed weight vectors in the same way as in MOEA/D\cite{MOEAD}\cite{hisao:dynamic}. 
...
Given the number of objectives $m$ and the population size $\mu$, $H$ can be calculated as follows:
\begin{equation}
...
Very roughly speaking, multi-objective evolution by EMOAs can be separated into
two stages:
...
So the primary task is to distribute the solutions more evenly on the Pareto front.
...
Thus, this process is called dynamic reference point specification. 
...
In this section,
a new mechanism that uses a weak convergence detection criterion to 
decide the timing of changing the value of $r$ from $r_{Initial}$ to $1+1/H$ is introduced. 
...
The number of objectives ($m$) is set as 10 in all test problems, 
and the number of decision variables ($D$) is specified for each test problem as follows: 
...
Stopping condition: 100,000 function evaluations, 
...
The results highlighted in bold in each row is the best result, and the worst result is shaded. 
The one-tailed Wilcoxon rank sum test is used to show the statistical significance for SMS-EMOA-10, SMS-EMOS-Opt, SMS-EMOA-LD in comparison with the proposed SMS-EMOA-CD. 
...
The one-tailed Wilcoxon rank sum tests show that almost all the results are ``$\approx$''. 
This is because for the triangular Pareto front, the location of the reference point (except the same as nadir point) has no influence on the optimal distribution of solutions. 
For WFG8 and WFG9, better results are obtained from SMS-EMOA-LD than SMS-EMOA-CD. 
For all the other test problems, these two algorithms have no statistically significant differences.
These observations suggest that SMS-EMOA-LD is slightly better than SMS-EMOA-CD on triangular Pareto front test problems. 
...
The one-tailed Wilcoxon rank sum tests show that SMS-EMOA-10 is significantly outperformed by SMS-EMOA-CD on 11 out of the 14 test problems.
...
This phenomenon is not observed for triangular Pareto fronts. 
That is, well-distributed solutions are obtained when the reference point is not just the nadir point of the triangular Pareto fronts. 
...
In the future, we will investigate the behavior of our new mechanism and further improve it. 
