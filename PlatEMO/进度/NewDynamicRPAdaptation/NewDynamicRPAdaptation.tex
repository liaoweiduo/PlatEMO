...
In 2014, a simple and fast version of SMS-EMOA, known as FV-MOEA\cite{FVEMOA}, has been proposed to further increase the efficiency.
FV-MOEA considers the fact that 
...
It has been reported that the position of the reference point strongly influences the hypervolume optimal distribution 
on the inverted-triangular PF\cite{hisao:RPhowtoSpecify, hisao:RPspecify, hisao:RPexplanation}. 
Thus, a reference point specification method is proposed in \cite{hisao:RPhowtoSpecify} for fair HV computation.
...
presented in the experiment section. On some specific problems (e.g., Distance Minimization Problems\cite{dmp}), 
...
\begin{figure}[!t]
  ...
  which are far away from the reference point.}
  \label{rpa1}
\end{figure}
...
and many solutions in the final solution set will distribute on the boundary of the PF
...
\begin{figure}[!t]
  ...
  \caption{The final distribution of the solution set on 3-objective inverted-DTLZ1((a) and (b)) 
  and 3-objective DTLZ1 ((c) and (d)).
...
\begin{figure}[!t]
  ...
  \caption{The final distribution of the solution set on 3-objective inverted-DTLZ1.
...
\begin{figure}[!t]
  ...
  \caption{
    ...
    The population size $\mu = 30$ ($H=1$), the total evaluation number $= 10,000$.
  }
  \label{nud}
\end{figure} 
...
When the algorithm reaches the final stage of the algorithm, all solutions are near the PF. 
...
The sensitivity of the $r$ value on the solution distribution is also observed on some real-world problems,
...
In this section,
we will introduce a new mechanism that uses the weak convergence detection criterion to 
...
\begin{figure}[!t]
  ...
  (window size $w\_l = 4,000$ and population size $\mu = 100$).
...
The whole process of the weak convergence detection is also described in Algorithm \ref{alg:wcd}. 
...
Finally, the HV of SMS-EMOA-CD is better than SMS-EMOA-10, SMS-EMOA-Opt and SMS-EMOA-LD. 
